...
SELECT is(
  (SELECT one_rep_max_weight_id FROM public.user_exercise_weights WHERE user_id = '00000000-0000-0000-0000-000000000002' AND exercise_type = 'barbell_bench_press'),
  NULL::uuid,
...
SELECT public.update_user_one_rep_max('00000000-0000-0000-0000-000000000002', 'barbell_bench_press', 225, 'pounds');

-- 2. Check that the one_rep_max_weight_id is set and points to the correct weight
SELECT ok(
  (SELECT w.weight_value = 225 AND w.weight_unit = 'pounds'
...
SELECT public.update_user_one_rep_max('00000000-0000-0000-0000-000000000002', 'barbell_bench_press', 250, 'pounds');

-- 4. Check that the one_rep_max_weight_id is updated to the new value
SELECT ok(
  (SELECT w.weight_value = 250 AND w.weight_unit = 'pounds'
   FROM public.user_exercise_weights uew
   JOIN public.weights w ON w.id = uew.one_rep_max_weight_id
   WHERE uew.user_id = '00000000-0000-0000-0000-000000000002' AND uew.exercise_type = 'barbell_bench_press'),
