...
-- block. (But also make this work for existing blocks that are done too lol)
          ...
          SELECT ROW(
            ...
            ex.performed_at,
              COALESCE(
                ...
                CASE
                  WHEN ebe.exercise_order = 1 THEN (
                    ...
                    WHERE esbb2.superblock_id = esb.id
                      ...
                      AND ex2.id <> ex.id
                      AND (
                        ex.performed_at IS NULL OR ex2.performed_at < ex.performed_at
                      )
