...
BEGIN
    ...
    INSERT INTO public.exercise_superblock (
        ...
        v_superblock_id, p_user_id, 'Leg Day', NULL, timezone('utc', now()), timezone('utc', now())
    );

    -- Create Wendler block (barbell_squat)
    v_wendler_block_id := public.wendler_exercise_block(p_user_id, p_training_max, 'barbell_squat', p_wendler_cycle);
    ...
    v_leg_curl_block_id := public.add_exercise_block(p_user_id, 'machine_seated_leg_curl', 'machine', 0, 5, 15);
    INSERT INTO public.exercise_superblock_blocks (superblock_id, block_id, superblock_order)
    VALUES (v_superblock_id, v_leg_curl_block_id, v_order);
    v_order := v_order + 1;

    -- Create machine_leg_extension block (5x15)
    v_leg_extension_block_id := public.add_exercise_block(p_user_id, 'machine_leg_extension', 'machine', 0, 5, 15);
    INSERT INTO public.exercise_superblock_blocks (superblock_id, block_id, superblock_order)
    VALUES (v_superblock_id, v_leg_extension_block_id, v_order);
    v_order := v_order + 1;

    -- Create plate_stack_calf_raise block (5x15)
    v_calf_raise_block_id := public.add_exercise_block(p_user_id, 'plate_stack_calf_raise', 'plate_stack', 0, 5, 15);
    INSERT INTO public.exercise_superblock_blocks (superblock_id, block_id, superblock_order)
