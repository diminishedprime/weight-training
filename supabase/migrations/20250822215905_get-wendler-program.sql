...
DROP TYPE IF EXISTS public.get_wendler_program_result;

DROP TYPE IF EXISTS public.p_wendler_cycle_row;

DROP TYPE IF EXISTS public.p_wendler_movement_row;
...
BEGIN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'p_wendler_cycle_row') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'get_wendler_program_result') THEN
