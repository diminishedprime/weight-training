...
CREATE OR REPLACE FUNCTION public.get_superblocks_for_user (p_user_id uuid) RETURNS TABLE (
  id uuid,
  user_id uuid,
  name text,
  notes text,
...
CREATE OR REPLACE FUNCTION public.get_exercise_blocks_for_superblock (p_superblock_id uuid) RETURNS TABLE (
  block_id uuid,
  block_order integer,
  name text,
  notes text,
