-- Delete all existing Wendler program data to ensure clean migration
DELETE FROM public.wendler_program_cycle_movement;

DELETE FROM public.wendler_program_cycle;

DELETE FROM public.wendler_movement_max;

DELETE FROM public.wendler_program;

-- Drop existing functions to recreate them with new signatures
DROP FUNCTION IF EXISTS public.add_wendler_program (
  uuid,
  numeric,
  numeric,
  numeric,
...
DROP FUNCTION IF EXISTS _impl.add_cycle (
  uuid,
  uuid,
  public.wendler_cycle_type_enum,
  numeric,
  numeric,
  numeric,
  numeric,
...
DROP FUNCTION IF EXISTS _impl.add_movement (
  uuid,
  uuid,
  public.wendler_cycle_type_enum,
  exercise_type_enum,
...
DROP FUNCTION IF EXISTS _impl.add_block (
  uuid,
  uuid,
  exercise_type_enum,
  public.wendler_cycle_type_enum,
...
DROP FUNCTION IF EXISTS _impl.add_exercise (
  uuid,
  uuid,
  exercise_type_enum,
  equipment_type_enum,
...
DROP FUNCTION IF EXISTS _impl.create_movement_max (uuid, numeric, numeric, weight_unit_enum);

-- Recreate functions with cycle_order support
