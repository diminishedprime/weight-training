...
const calc3x3Sets = (weight: Weight_V1, warmupSet: WarmupSet): BarSet[] => {
  const sets: BarSet[] = [];
...
type _3x3Data = NotStarted | InProgress;
...
const use3x3 = (exercise: BarExercise) => {
  const addExercise = useAddExercise();

  const xform = useCallback((a: _3x3Data) => {
  ...
  const [data, setData] = usePersistentObject<_3x3Data>(
    ObjectKey._3x3Data,
    notStarted(),
    ...
    (s: string | undefined) => {
      try {
        ...
        setData((old) => ({
          ...old,
          orm: s,
          targetWeight: Math.floor(n * _3x3ORMRatio).toString(),
        }));
      } catch (e) {
    ...
    (s: string | undefined) => {
      try {
        ...
        setData((old) => ({
          ...old,
          targetWeight: s,
          orm: Math.floor(n / _3x3ORMRatio).toString(),
        }));
      } catch (e) {
        console.error(`Could not parse "${s}" as a number`);
      }
  ...
  useEffect(() => {
    ...
    setData((old) => ({
      ...
      targetWeight: Math.floor(
        actualORM.weight.value * _3x3ORMRatio,
      ).toString(),
    }));
  }, [actualORM, setData]);
  ...
  const start3x3 = useCallback(() => {
    ...
    try {
      ...
      const sets = calc3x3Sets(actualWeight, data.warmupSet);
