...
import { minimalPlates, correspondingEquipment, requireId } from './util'; // Removed getSupabaseClient
...
describe('util', () => {
  ...
  describe('correspondingEquipment', () => {
    ...
    it('should return "plate_stack" for plate_stack exercises', () => {
      expect(correspondingEquipment('plate_stack_calf_raise')).toBe('plate_stack');
    });
  ...
  describe('requireId', () => {
    ...
    it('should call redirect if session is null', () => {
      ...
      expect(navigationRedirectMock).toHaveBeenCalledWith(`/login?redirect-uri=${encodedPath}`);
    });
